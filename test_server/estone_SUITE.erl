...
run_micro(Top, M, DataDir) ->
    ...
    io:format("computed EstoneCat=~p~n", [EstoneCat]),
...
ppp(Top, I, EstoneCat) ->
    io:format("launching ~p~n", [EstoneCat]),
...
ppp_loop(_P, 0, _) ->
    ok;
ppp_loop(P, I, Cmd) ->
%    io:format("sending ~p to ~p~n", [P,Cmd]),
    P ! Cmd,
    receive
	{P, _} ->  %% no match
	    ppp_loop(P, I-1, Cmd)
...
g_loop(0) ->
    io:format("Killing ~p~n", [whereis(funky)]),
...
req(Name, Req) ->
    R = make_ref(),
    Name ! {self(), R, Req},
    %io:format("sending to ~p(~p) ! ~p~n", [Name,whereis(Name),{self(), R, Req}]),
    receive
	{Name, R, Reply} -> Reply;
	Msg -> io:format("got bogus message ~p, expected ~p~n", [Msg, {Name, R, '_'}]),
	       io:format("server: ~p~n", [erlang:process_info(whereis(Name), messages)]),
	       erlang:exit(badmsg)
    after 2000 ->
	       io:format("server: ~p~n", [erlang:process_info(whereis(Name))]),
	    exit(timeout)
    end.
...
gserv(Name, Mod, State, Debug) ->
	...
	{From, Ref, {call, Req}} when Debug == [] ->
		...
		{reply, Reply, State2} ->
		    %io:format("replying to ~p~n", [From]),
		    From ! {Name, Ref, Reply},
		    gserv(Name, Mod, State2, Debug);
		{noreply, State2} ->
		    io:format("noreply to ~p~n", [From]),
		    gserv(Name, Mod, State2, Debug);
		{'EXIT', Reason} ->
		    io:format("server death to ~p~n", [From]),
		    exit(Reason)
	    end;
