...
heir_founder(Master, HeirData, Opts) ->    
    ...
    ?line case ets:info(T,heir) of
	      none ->
		  io:format("case 1 Heir=~p~n", [Heir]),
		  ?line true = (Heir =:= none) orelse (not is_process_alive(Heir)),
		  Master ! {"No heir",self()};
	      
	      Heir -> 
		  io:format("case 2 Heir=~p~n", [Heir]),
...
heir_heir(Founder, Mode) ->
    ...
    io:format("xx2 expecting ~p~n", [{'ETS-TRANSFER', T, Founder, HeirData}]),
...
match1_do(Opts) ->
    ...
    io:format("1 ~p~n", [Opts]),
    ?line case ets:match(Tab,{{one,'_'},'$0'}) of
	      [[4],[5]] -> ok;
	      [[5],[4]] -> ok
	  end,
    io:format("2 ~p~n", [Opts]),
...
match2_do(Opts) ->
    ...
    ?line case ets:match(Tab, {value2_1, '$2'}) of
	[[key2],[key2_wannabe]] -> ok;
	[[key2_wannabe],[key2]] -> ok;
	_ -> ?t:fail("ets:match(Tab, {value2_1, '$2'}) returned something funny.")
    end,
